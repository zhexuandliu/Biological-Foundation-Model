...
# set2condition <- rjson::fromJSON(file = file.path("../data/GEARS/replogle_k562_conditions.json"))
...
# 
# 
# observed <- assay(psce, "change")[na.omit(gene_matches), , drop=FALSE]
# 
# train_indices <- psce$training == "train"
# mse_train <- mean((pred[, train_indices] - observed[, train_indices])^2, na.rm = TRUE)
# 
# test_indices <- psce$training == "test"
# mse_test <- mean((pred[, test_indices] - observed[, test_indices])^2, na.rm = TRUE)
# 
# print(mse_test)
...
# Set up argument parser (you can hardcode values for testing)
pa <- list(
  gene_embedding = "training_data",  # Change this to "random" to test
  pert_embedding = "random",  # Change this to "random" to test  
  pca_dim = 10,
  ridge_penalty = 0.1,
  seed = 1  # IMPORTANT: Set seed for reproducibility
)
...
cat("Data dimensions:\n")
cat("- Total perturbations:", ncol(psce), "\n")
cat("- Training perturbations:", ncol(train_data), "\n")
cat("- Test perturbations:", sum(psce$training == "test"), "\n")
cat("- Genes:", nrow(psce), "\n")

# Create embeddings using pa arguments
gene_emb <- if(pa$gene_embedding == "training_data"){
  cat("Creating PCA gene embeddings from training data\n")
  pca <- irlba::prcomp_irlba(as.matrix(assay(train_data, "X")), n = pa$pca_dim)
  rownames(pca$x) <- rownames(train_data)
  pca$x
}else if(pa$gene_embedding == "random"){
  cat("Creating random gene embeddings\n")
  tmp <- matrix(rnorm(nrow(psce) * pa$pca_dim), nrow = nrow(psce), ncol = pa$pca_dim)
  rownames(tmp) <- rownames(psce)
  tmp
}else{
  stop("Unsupported gene embedding type")
}

pert_emb <- if(pa$pert_embedding == "training_data"){
  cat("Creating PCA perturbation embeddings from training data\n")
  pca <- irlba::prcomp_irlba(as.matrix(assay(train_data, "X")), n = pa$pca_dim)
  rownames(pca$x) <- rownames(train_data)
  t(pca$x)
}else if(pa$pert_embedding == "random"){
  cat("Creating random perturbation embeddings\n")
  tmp <- matrix(rnorm(ncol(psce) * pa$pca_dim), nrow = pa$pca_dim, ncol = ncol(psce))
  colnames(tmp) <- psce$clean_condition
  tmp
}else{
  stop("Unsupported perturbation embedding type")
}

# Ensure control condition is included
if(! "ctrl" %in% colnames(pert_emb)){
  pert_emb <- cbind(pert_emb, ctrl = rep(0, nrow(pert_emb)))
}
...
cat("- Perturbation matches:", sum(!is.na(pert_matches)), "/", length(pert_matches), "\n")
cat("- Gene matches:", sum(!is.na(gene_matches)), "/", length(gene_matches), "\n")
...
cat("\nResults:\n")
cat("Gene embedding:", pa$gene_embedding, "\n")
cat("Perturbation embedding:", pa$pert_embedding, "\n")
